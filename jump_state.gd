...
func enter() -> void:
	...
	_actor.velocity.x = 0
	_actor.velocity.y = 0
	_actor.velocity.y += _jump_settings.get_velocity(false)
...
func _check_if_double_jumping() -> void:
	if Input.is_action_just_pressed("jump"):
		_current_state = State.Idle
		exited.emit(MovementStateMachine.State.DoubleJump)
...
func _process(delta: float):
	...
	if _current_state == State.Falling and _actor.is_on_floor():
		_on_touched_floor()
		return
